...
`{default_death}`, `{ec_balance}`, `{ec_playtime}`.
...
`{ec_balance}` and `{ec_playtime}` pull straight from EndstoneCore if
it's installed alongside this plugin (entry point name `core`) - they
show `N/A` otherwise.
