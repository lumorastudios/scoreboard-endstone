...
class ScoreboardPlugin(Plugin):
    ...
    authors = ["Appolo"]
    # Not required to run, but if it's installed we want it loaded first so
    # {ec_balance}/{ec_playtime} work from the very first render.
    soft_depend = ["core"]
    prefix = "Scoreboard"
    ...
    def on_enable(self) -> None:
        ...
        self.stats = StatsStore(Path(self.data_folder) / "stats.json")
        register_defaults(self.api, self.server, self.stats)
    ...
    def on_player_death(self, event: PlayerDeathEvent) -> None:
        victim = event.player
        self.stats.add_death(str(victim.unique_id))

        # damage_source.actor is whoever gets credit for the kill - already
        # resolved to the shooter for projectiles, not the arrow itself.
        source = event.damage_source
        killer = source.actor if source else None
        if isinstance(killer, Player) and killer.unique_id != victim.unique_id:
            self.stats.add_kill(str(killer.unique_id))

    # ------------------------------------------------------------------
    # board building / refresh loop
    # ------------------------------------------------------------------
